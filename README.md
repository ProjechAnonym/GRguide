...
## 环境依赖
### 后端
后端依赖redis作账号找回的身份验证功能,同时在二进制文件所在目录下应该确保存在config/config.yaml,log文件夹,static文件夹,其中log和static应确保有读写的权限
### 前端
前端需要在下载nodejs的环境中打包,进入项目所在目录之后输入npm install就可以直接下载依赖,打包后的文件可以由nginx部署
